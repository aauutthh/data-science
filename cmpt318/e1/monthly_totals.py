...
# pd_summary.py
# CMPT 318 Exercise 1 - Analysis with Pandas
# Alex Macdonald
# ID#301272281
# September 11, 2018
...
def pivot_months_pandas(data):
    ...
    # 1. Add a column 'month' that contains the results of applying the
    # date_to_month function to the existing 'date' column
    # You may have to modify date_to_month slightly, depending how your data types work out    
    monthColumn = data['date'].apply(date_to_month)
    data['month'] = monthColumn

    # 2. Use the Pandas groupby method to aggregate over the name and month columns
    # Sum each of the aggregated values to get the totals
    # Hint: grouped_data.aggregate('sum').reset_index()
    grouped_data_sum = data.groupby(['name', 'month']).aggregate('sum').reset_index()

    # 3. Use the Pandas pivot method to create a row for each station (name) 
    # and column for each month
    monthly = grouped_data_sum.pivot(index='name', columns='month', values='precipitation')

    # 4. Repeat with the 'count' aggregation to get the count of observations
    grouped_data_count = data.groupby(['name', 'month']).aggregate('count').reset_index()
    counts = grouped_data_count.pivot(index='name', columns='month', values='precipitation')
...
def main():
    ...
    # totals, counts = pivot_months_loops(data) # using the loop method
    totals, counts = pivot_months_pandas(data) # using the pandas method
